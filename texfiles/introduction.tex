...
\ds{Intro is a bit long -- need to tighten.}
...
As a consequence, OS policy design is becoming increasingly workload-dependent.
...
CBMM observes that Linux promotes hugepages too indiscriminately, and instead promotes them only when the estimated allocation cost is lower than the estimated benefit, with the latter obtained from profiling the workload over several runs.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/Overview/CBMM-example.pdf}
...
\ds{Possible high-level bit: why does this only apply to the OS? I guess it applies generally but we are instantiating only in the OS because it is a rich space of policies, and has a bunch of issues today?}
...
As we explain in \Cref{subsec:principles},  this is important for run-time introspection, but it also means developers must translate high-level goals into monitorable properties.
For example, in the case of CBMM, developers must transform their high-level objective of making better hugepage promotion decisions than Linux into monitorable properties, such as requiring the estimated benefits to match the observed benefits.
Further, the prescribed corrective action may or may not guarantee the desired end-to-end performance property. 
The challenge, thus, is to ensure that the prescribed properties and actions \aditya{properties and actions both? result in meeting the high-level objective.} \aditya{objective brought in for the first time? is this referring to "non degradation" type "objectives"?} 
