...
The Operating System (OS) lies at the core of modern computing.
The OS manages hardware and software resources through policies that govern resource allocation and control.
Conventionally, OS policies have been designed for generality -- resulting in largely static OS policies with only coarse knobs (e.g., turning hugepages on/off, or choosing nice values for tasks) to tune performance for specific applications manually.
Such static policies are increasingly ill-suited to the modern computing landscape: applications are evolving rapidly, have tight performance requirements, and workloads are highly dynamic; also, modern infrastructure is deeply heterogeneous, combining multiple classes of CPU cores, memory tiers, and accelerators on a single system.
For instance, the CFS scheduler can result in high performance variability for certain application classes that can generate imbalance in requests to threads~\cite{syrup-sosp21,shinjuku-nsdi19}, result in non-work-conserving behavior for specific hardware~\cite{wasted-cores}, or degrade performance when large number of CPUs share the same domain~\cite{meta-lavd}. \aditya{the example later switches to memory/cbmm. can we replace this with a memory example?}
\eric{I think we can start with a punchier intro sentence, something that conveys: "static, general purpose OS policies have gone the way of the dodo"}

As a consequence, OS policy design is becoming increasingly workload-dependent. \sujay{such as learned policies, LLM-generated heuristics.}
\eric{this would be a good time to introduce what this means precisely: policies are optimized w.r.t. expected workloads.}
...
\eric{The authors observe?} CBMM observes that Linux promotes hugepages too indiscriminately, and instead promotes them only when the estimated benefit is higher than the estimated allocation cost, with the former obtained from profiling the workload over several runs.
\eric{... allocation cost. Crucially, "estimated benefit" is a prediction about the future performance benefits of promoting a hugepage, and is computed based on data profiled offline, i.e. prior to deployment time.}
This design assumes that the benefits seen during offline profiling remain valid at deployment time -- this assumption can break when different regions become hot over time or when deployment-time access patterns differ from those seen during profiling.
\eric{Designs like this that rely on offline-profiled data work under the assumption that the benefits seen during offline profiling remain valid at deployment time -- ....}
...
We propose an alternative design philosophy of deploying and refining OS policies, based on {\it \textbf{performance introspection}} -- the ability to check {\em during execution} whether OS policies are delivering the desired performance. \aditya{we want to clarify how this is different from/complementary with runtime verification}
We realize this philosophy through {\it \textbf{OS guardrails}}, an abstraction that {\em requires} policy developers (or OS operators) to specify: (i) the desired performance property as an expression over execution-time statistics, and (ii) a corrective action to invoke when that property is violated.
\eric{We haven't really said what the `philosophy' is, only what its based on. If the philosophy is ``write guardrails'', then guardrails really \emph{are} the philosophy. If the policy is ``check local, repair, ensure global'', then we need to say that in a punchy way before this sentence.  }
Rather than treating the satisfaction of desired performance properties as a `system management' concern -- something developers evaluate only after designing policies -- our framework forces developers to choose execution-monitored performance properties, elevating them to first-class `system design' concern. \aditya{This sentence is important, but seems out of place}
\eric{perhaps this sentence is the punchy framing of the philosophy?}
...
Besides providing run-time monitoring and correction capabilities, guardrails can also be used to provide visibility into policy behavior: they help identify when and under what conditions a policy fails to meet the desired properties.
\eric{This is treated as a claim, but feels like it hasn't been justified yet. It needs something like "our case studies will show", or "in our experience". Or it needs an analytical justification}
...
Revisiting the example of the hugepage promotion policy in CBMM, a desired \eric{desirable?} performance property is
\eric{\st{to ensure}} that the benefits estimated during profiling are close to the actual benefits obtained by promoting the specific hugepages at deployment time (see \Cref{fig:intro-example}) \eric{Is this the property that we verify/validate in the case study?}, and the corrective action could iteratively improve the CBMM policy by updating the profiled benefits based on the observed ones. \aditya{this guardrail and action seem a little weak for the intro, tbh}
...
However, realizing the guardrail abstraction in the OS kernel today requires us to address multiple challenges. First, different OS policies invoke different types of performance requirements – e.g., the hugepage promotion policy from the above example would require the estimated benefits to be accurate, a load balancer would require low core imbalance, and a learned flow size predictor would require accurate predictions.
...
\ds{Put somewhere:
Recent efforts to customize various OS subsystems via eBPF~\cite{kflex-sosp24,cache-ext-sosp25,sched-ext,meta-lavd} or support bespoke OS policies in userspace~\cite{ghost-sosp21} reflect this need for workload-specific behavior.
Similarly, learning-driven OS policies~\cite{heimdall-eurosys25,orca-sigcomm20,kml-tos23,kleio-hpdc19} also aim to produce decisions optimized for the current workload by leveraging large amounts of training data.
